...
export default function EditAlbumPage() {
  ...
  const { data: album, isLoading: isAlbumLoading } = useQuery({
    ...
    onSuccess: (data) => {
      // Set form values from album data
      form.reset({
        title: data.title,
      ...
      setSelectedCoverImage(data.coverImageId || null);
    },
  });
  ...
  const updateAlbumMutation = useMutation({
    mutationFn: async (data: AlbumFormData) => {
      const res = await apiRequest(`/api/albums/${albumId}`, "PATCH", data);
      return res.json();
    },
    onSuccess: () => {
      toast({
        title: "Album updated",
        description: "Your album has been successfully updated",
      });
  ...
  // Add image to album mutation
  const addImageToAlbumMutation = useMutation({
    mutationFn: async (imageId: number) => {
      const res = await apiRequest(`/api/albums/${albumId}/images`, "POST", { imageId });
      return res.json();
    },
    onSuccess: () => {
      toast({
        title: "Image added",
        description: "Image has been added to the album",
      });
      queryClient.invalidateQueries({ queryKey: [`/api/albums/${albumId}`] });
    },
    onError: (error: Error) => {
  ...
  const removeImageFromAlbumMutation = useMutation({
    mutationFn: async (imageId: number) => {
      const res = await apiRequest(`/api/albums/${albumId}/images/${imageId}`, "DELETE");
      return res;
    },
    onSuccess: () => {
      toast({
  ...
  const onSubmit = (data: AlbumFormData) => {
    ...
    updateAlbumMutation.mutate(data);
  };
  ...
  const handleAddImage = (image: Image & { user: { name: string } }) => {
    // Check if image is already in album
    const isImageInAlbum = album?.images?.some((albumImage: any) => albumImage.id === image.id);
    
    if (isImageInAlbum) {
      // If already in album, remove it
      removeImageFromAlbumMutation.mutate(image.id);
    } else {
      // Otherwise add it
      addImageToAlbumMutation.mutate(image.id);
    }
  };
  ...
  // Check if album images are loaded
  const albumImages = album?.images || [];
  
  return (
    <div className="container mx-auto py-8 px-4">
      <Button variant="ghost" onClick={() => navigate(`/albums/${albumId}`)} className="mb-6">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="pt-6">
                <Button
                  type="submit"
                  disabled={updateAlbumMutation.isPending || !form.formState.isDirty}
                >
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-gray-600 mb-4">Select an image to use as the album cover.</p>
        ...
        <div>
          <h2 className="text-xl font-semibold mb-4">Manage Album Images</h2>
          <p className="text-gray-600 mb-4">Click on images to add or remove them from the album.</p>
          ...
          {isImagesLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {userImages.map((image: any) => {
                const isInAlbum = albumImages.some((albumImage: any) => albumImage.id === image.id);
                return (
                  <div
                    key={image.id}
                    className={`relative aspect-square rounded-md overflow-hidden cursor-pointer border-2 ${
                      isInAlbum ? "border-primary" : "border-gray-200"
                    }`}
